let app = {
    ...
    svg: {
         ...
         * @param element Element DOM ID for inject the SVG
         * @param width SVG BOX width
         * @param height SVG BOX height
         * @param coord String of coordinate of path
         * @returns {{paper: *, svgPath}} paper: SVG BOX, SvgPath : path object
         */
        createPath: function(element, width, height, coord) {

            // Instance of SVG
            let paper = new Raphael(document.getElementById(element), width, height);

            // Path coordinates
            let svgPath = paper.path(coord);

            //Return the node Svg and the node Path
            return {
